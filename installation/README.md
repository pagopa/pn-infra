...
- Comunicare ai gestori dell'account di Continuos Integration (Team Piattaforma Notifiche core) 
...
- Profilo __dns_delegator__: per definire questo profilo è necessario chiedere delle credenziali 
  ...
  _Preparazione Networking_._Zone DNS pubbliche_ per creare una delega verso l'abiente che si
...
- Definire networking tra account dello stesso ambiente come da diagramma
  ![Networking interno ad un installazione di Piattaofrma Notifiche](hl_network_infra.drawio.png).
...
### Procedimento
  - Effetturare il clone del reposiutory github [pn-infra](https://github.com/pagopa/pn-infra)
  ...
  - Eseguire lo script bash [create_and_share_private_hosted_zones.sh](../scripts/prepare-networking/private_hosted_zones/create_and_share_private_hosted_zones.sh) 
    ...
    ```
  - `<id-della-vpc-privata-account-core>` deve essere sostituiro col il _VPC id_ della VPC
    privata presente nell'account _PN-CORE_.
  - `<id-della-vpc-privata-account-helpdesk>` deve essere sostituiro col il _VPC id_ della VPC
    privata presente nell'account _HELPDESK_.
  - `<id-della-vpc-privata-account-confidential-info>` deve essere sostituiro col il _VPC id_ 
...
## Scopo
Installare il sistema di login utlizzato dai destinatari delle notifiche.
...
nuovo ambiente (ad esempio cert) nella cartella pn-cicd/cd-cli/custom-config/pn-data-vault. Le 
configurazioni sono composte da due file. 
- `cd-cli/custom-config/pn-data-vault/scripts/aws/cfn/once4account/coll.yaml` che va 
  ricopiato in `cd-cli/custom-config/pn-data-vault/scripts/aws/cfn/once4account/cert.yaml` e volendo 
  va customizzato per personalizzare l'invio degli allarmi su slack o per mail. Fondamentale è mantenere 
  gli output esistenti.
- `cd-cli/custom-config/pn-data-vault/scripts/aws/cfn/microservice-coll-cfg.json`che va ricopiato in
  `pn-cicd/cd-cli/custom-config/pn-data-vault/scripts/aws/cfn/microservice-cert-cfg.json` e modificato
  ...
  - __VpcSubnets__: id delle sottoreti PAGOPA-CERT-CONFIDENTIALINFO-GENERIC-A, PAGOPA-CERT-CONFIDENTIALINFO-GENERIC-B, PAGOPA-CERT-CONFIDENTIALINFO-GENERIC-C
...
Tutte le operazioni vanno eseguite nell'account _CONFIDENTIAL-INFORMATION_ nella regione eu-south-1
...
- Nel bucket _CdArtifactBucket_ caricare:
  ...
  - e il file _desired-commit-ids-env.sh_ preparato in precedenza nella cartella _config_
...
Richiedere al team di front-end di aggiungere un pacchetto dedicato all'ambiente specifico.
...
allo stato attuale, sono ottenibili da quelle di un altro ambiente sostituendo il nome dell'ambinete 
vecchio con il nuovo. Inoltre vanno modificati i seguenti parametri:
...
__N.B.:__ Una volta aggiornate le configurazioni il repository va aggiornato e memorizzato il __commit-id__
...
## Procedimento di installazione
Tutte le operazioni vanno eseguite nell'account _PN-CORE_ nella regione eu-south-1
- Definire un secret contenente le necessarie API-Key seguendo quanto descritto nella pagina
